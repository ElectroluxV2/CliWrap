        ...
        [EnumeratorCancellation] CancellationToken forcefulCancellationToken)
    {
        ...
        var stdOutPipe = PipeTarget.Merge(
            ...
            PipeTarget.Create(async (stream, token) =>
            {
                ...
                {
                    await channel.PublishAsync(new StandardBinaryOutputCommandEvent(buffer[..bytesRead]), token).ConfigureAwait(false);
                }
            })
        );
        ...
        var stdErrPipe = PipeTarget.Merge(
            ...
            PipeTarget.Create(async (stream, token) =>
            {
                ...
                while ((bytesRead = await stream.ReadAsync(buffer, token).ConfigureAwait(false)) != 0)
                {
                    await channel.PublishAsync(new StandardBinaryErrorCommandEvent(buffer[..bytesRead]), token).ConfigureAwait(false);
                }
            })
        );
